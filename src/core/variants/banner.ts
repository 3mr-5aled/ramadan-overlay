 ...
 *  - `bannerTextColor` — greeting text color  (falls back to colors[0])
 *  - `bannerIconColor` — lantern icon color   (falls back to colors[1])
 *  - `bannerText`      — custom greeting text (falls back to locale default)
 *  - `locale`          — 'en' | 'ar'
...
function buildLanternIcon(color: string): string {
  return `<svg viewBox="0 0 54.700001 119.00001" height="36" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style="flex-shrink:0;display:block">
    <g transform="matrix(0.1,0,0,-0.1,-112.1,290)" fill="var(--ro-banner-icon, ${color})" stroke="none">
...
function buildBar(
  ...
  text.style.cssText = [
    `color:var(--ro-banner-text, ${textColor})`,
    `font-family:${isArabic ? "'Amiri','Scheherazade New',Georgia,serif" : "Georgia,'Times New Roman',serif"}`,
    ...
    `line-height:1.3`,
    `letter-spacing:${isArabic ? "0.02em" : "0.04em"}`,
  ].join(";");
...
export function mountBannerElements(
  ...
  // Save original body padding so we can restore on destroy
  const origPaddingTop = document.body.style.paddingTop;
  ...
  if (showTop) {
    ...
    el.style.setProperty("--ro-banner-bg", bg);
    el.style.setProperty("--ro-banner-text", textColor);
    el.style.setProperty("--ro-banner-icon", iconColor);
    ...
    document.body.style.paddingTop = `${current + BANNER_HEIGHT}px`;
  }
